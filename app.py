...
def get_user_input():
    ...
    for i in range(4):  # Repeat 4 times
        while True:
            ...
            if (all(len(s) == 1 and s.isalpha() for s in one_side_set) and 
                len(one_side_set) == 3 and 
                one_side_set.isdisjoint(used_letters)):  
                
                sides.append((set1, set2, set3))
                used_letters.update(one_side_set)  # Add new letters to the used set
                break  # Move to the next entry
            else:
                print("Invalid input! Ensure all are single unique letters, and no duplicates across all entries.")
...
def find_valid_word_pairs(filtered_word_list, user_entries, full_letter_set):
    ...
    for i, word1 in enumerate(filtered_word_list):
        for j, word2 in enumerate(filtered_word_list):
            if i != j and is_valid_word_pair(word1, word2, user_entries, full_letter_set):
